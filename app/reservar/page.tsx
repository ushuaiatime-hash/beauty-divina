...
export default function ReservarPage() {
  ...
  async function fetchBookedSlots() {
    ...
    const { data } = await supabase
      ...
      .eq("professional_name", selectedProfessional.name + " - " + selectedProfessional.specialty)
  ...
  async function handleReservar() {
    ...
    const professionalFullName = selectedProfessional.name + " - " + selectedProfessional.specialty;
    
    const { error: dbError } = await supabase.from("appointments").insert([{
      client_name: clientName,
      client_phone: fullPhone,
      service_name: selectedService.name,
      professional_name: professionalFullName,
      date: selectedDate,
      time: selectedTime,
      ...
      <main style={{ ...styles.main, maxWidth: 500 }}>
        ...
        {step === 2 && (
          <div style={styles.stepWrap} className="fadeIn">
            ...
            {/* Para Micaela Cosmiatra */}
            {selectedProfessional?.id === "micaela_cosmi" && (
              <div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: "#e91e63", marginBottom: 12, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  ✨ Estética Avanzada
                </h3>
            ...
            {/* Para Micaela Masajista */}
            {selectedProfessional?.id === "micaela_masajista" && (
              <div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: "#e91e63", marginBottom: 12, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  💆‍♂️ Masajes
                </h3>
            ...
            {selectedProfessional?.id !== "micaela_cosmi" && selectedProfessional?.id !== "micaela_masajista" && (
              <div style={styles.serviceGrid}>
                {selectedProfessional?.services.map((s: any) => (
                  <div key={s.id} style={{ ...styles.serviceCard, ...(selectedService?.id === s.id ? styles.serviceCardActive : {}) }} className="card-hover" onClick={() => { setSelectedService(s); setStep(3); }}>
                    <span style={styles.serviceIcon}>💅</span>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  progressLabel: { fontSize: 12, fontWeight: 500, transition: "color 0.3s" },
